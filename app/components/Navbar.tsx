...
export default function Navbar() {
  ...
  const cartCount = useCartStore((state) => state.cart.length);
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false); // <-- тут оголошення стану
...
<nav className="bg-[#fff] px-6 py-4 flex justify-between items-center sticky top-0 z-50">
  <Link
    href="/"
    className="text-xl flex items-center uppercase font-bold text-[#AA824D]"
  >
    Amali&apos;s Cake Shop Logo
  </Link>

  {/* Десктоп меню */}
  <div className="hidden md:flex items-center space-x-6 text-[12px] uppercase text-[#202020]">
    {navLinks.map((link) => (
      <Link
        key={link.href}
        href={link.href}
  ...
  {mobileMenuOpen && (
    <div className="absolute top-full left-0 w-full bg-white shadow-md flex flex-col items-center md:hidden py-4">
      {navLinks.map((link) => (
        <Link
          key={link.href}
          href={link.href}
          className={`py-2 w-full text-center ${
            pathname === link.href ? "bg-[#EFEADE]" : "hover:bg-pink-100"
          }`}
        >
          {link.label}
        </Link>
      ))}
      <Link href="/basket" className="relative py-2 w-full text-center">
